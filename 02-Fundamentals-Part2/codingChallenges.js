...
function checkWinner(avgDolphins, avgKoalas) {
    if (avgDolphins >= 2 * avgKoalas)      return `Dolphins Win's (${avgDolphins}) vs (${avgKoalas})`;
    else if (avgKoalas >= 2 * avgDolphins) return `Koalas Win's (${avgKoalas}) vs (${avgDolphins}).`;
...
const mark = { 
    name : "Mark Miller", weight : 78, height : 1.69, 
    bmi : function() {
...
const john = { 
    name : "John Smith", weight : 92, height : 1.95, 
    bmi : function() {
        return this.weight / (this.height ** 2);
    }
};
